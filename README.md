...
项目使用 `winston` 库实现了成熟的日志系统，具有以下特性：

### 日志功能
- 📝 **时间戳**: 每条日志都包含精确的时间戳
- 📍 **代码位置**: 自动显示文件名和行号
- 📊 **日志级别**: 支持 error、warn、info、debug、verbose 五个级别
- 📁 **文件输出**: 日志同时输出到控制台和文件
- 🔄 **日志轮转**: 自动管理日志文件大小和数量
...
### 日志格式示例
```
2025-07-26 18:16:22 [INFO] 这是一条信息日志
2025-07-26 18:16:22 [WARN] 这是一条警告日志
2025-07-26 18:16:22 [ERROR] 这是一条错误日志
2025-07-26 18:16:22 [DEBUG] 这是一条调试日志
```

### 日志级别
- `error`: 错误信息，会记录到 error.log
- `warn`: 警告信息
- `info`: 一般信息（默认级别）
- `debug`: 调试信息，会记录到 debug.log
- `verbose`: 详细调试信息
